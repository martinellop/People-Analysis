...
def Mahalanobis_distance(x:torch.Tensor, y:torch.Tensor, C:torch.Tensor):
    ...
    if diff.dim() > 1:
        ...
        dist = torch.sum(mat1 * diff)
    return torch.sqrt(dist)

# Mahalanobis distance? is it possible to have? --> it requires (means vector) and (covariance matrix) of the data distribution.
# it would be (probably) a great improvement for non-deep techniques.
